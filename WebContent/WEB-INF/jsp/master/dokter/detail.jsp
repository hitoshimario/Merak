...
<div>
	<fieldset><legend>Data Dokter</legend>
		...
		<form id="tambahData" name="form" action="<c:url value="/master/dokter/saveDokter"/>" method="post">
	
		<table>
			<tr>
				
				<td></td><td><input type="hidden" id="id" name="id" value="${dokter.id}" /></td> 
			</tr><tr>
				<td>NIP</td>
				<td><input type="text" class="input" id="nip_dokter" name="nip_dokter" value="${dokter.nip_dokter}" />
				</td>
			</tr><tr>	
				<td>Nama Depan</td><td><input type="text" id="nama_awal" name="nama_awal" value="${dokter.nama_awal}" /></td>
			</tr><tr>	
				<td>Nama Belakang</td><td><input type="text" id="nama_akhir" name="nama_akhir" value="${dokter.nama_akhir}" /></td>	
			</tr><tr>	
				<td>Jenis Kelamin</td>
				<td>
					<input type="radio" id="jns_kelamin_laki-laki" name="jns_kelamin" value="0" <c:if test="${dokter.jns_kelamin == '0'}">checked="checked"</c:if> />
					<label for="jns_kelamin_laki-laki">Pria</label>
					<input type="radio" id="jns_kelamin_perempuan" name="jns_kelamin" value="1" <c:if test="${dokter.jns_kelamin == '1'}">checked="checked"</c:if>/>
