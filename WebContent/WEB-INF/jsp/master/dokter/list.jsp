...
<fieldset><legend>Data Dokter</legend>
	<table class="display" class="table1" id="pagination" >
		...
		<tbody>
		<c:forEach items="${dokter}" var="dokter">  <!-- keynya, dan awalnya dokter --> 		
		<tr >
			...
			<td><c:out value="${dokter.jns_kelamin eq '0' ?'Laki-laki':'Perempuan' }" /></td>
			...
			<td width=20> <a class="back" href="<c:url value="/master/dokter/detail?id=${dokter.id}"/>"><i class="icon-pencil"> </i></a></td>
			<td><a class="back" href="<c:url value="/master/dokter/deleteDokter?id=${dokter.id}"/>"><i class="icon-remove"> </i></a></td>
				
		</tr>
		</c:forEach>
		</tbody>
	</table>
	<a class="button" href="<c:url value="/master/dokter/add"/>"><BUTTON>Tambah Dokter</BUTTON></a>
	</fieldset>
</div>
<div id="footer"></div>
</body>
